...
class c_mspacman : public c_pacman, register_class<system_registry, c_mspacman>
{
  public:
        ...
        return {
            {
                ...
                .input_info = c_pacman::get_input_info(),
    ...
    c_mspacman(PACMAN_MODEL model) : c_pacman(model)
    {
    ...
    int load()
    {
        ...
        std::vector<s_roms> mspacman_roms = {
            ...
            {"82s126.1m", 0xa9cc86bf,  256,      0, pacman_psg->sound_rom.get()},
            {"82s126.3m", 0x77245b66,  256,  0x100, pacman_psg->sound_rom.get()},
        };
  ...
  private:
    ...
    uint16_t bitswap(uint16_t in, std::array<uint8_t, 16> pos)
    {
        uint16_t out = 0;
        for (int i = 0; i < 16; i++) {
