    ...
    void generate_color_lookup()
    {
        ...
        if (color_lookup_built.compare_exchange_strong(expected, 1)) {
            ...
            for (int i = 0; i < 32; i++) {
                ...
                color_lookup[i] = std::clamp((uint32_t)(-.0148 * (x * x * x) + .6243 * (x * x) + 2.9731 * x),
...
private:
    ...
    static inline std::atomic<int> color_lookup_built = 0;
    ...
    static inline uint32_t color_lookup[32];
...
const unsigned int c_gbppu<Gb>::palettes[][4] = {
    {
        //0 - lime green
