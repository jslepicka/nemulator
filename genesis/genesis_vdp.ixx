...
constexpr auto make_rgb_entries()
{
...
export class c_vdp
{
  ...
  public:
    ...
    //|--------------------------------------------------------------------------------------|
    //|
    // asserted|VCounter changes |                 |      Never      |                 |
  ...
  private:
    ...
    void draw_plane(uint8_t *out, uint32_t nt, uint32_t plane_width, uint32_t plane_height, uint32_t v_scroll,
                    uint32_t h_scroll, uint32_t low_pri_val)
    {
        ...
        bool sh_mode_enabled = reg[0x0C] & 0x8;
        ...
        for (int column = 0; column < column_count; column++) {
            ...
            uint32_t pattern_address = tile_number * bytes_per_tile + (((y & 7) ^ v_flip) * bytes_per_tile_row);
            ...
            *((uint64_t *)&out[x]) = pixels;
            ...
            for (int p = 0; p < 8; p++) {
                ...
                if (priority) {
                    priority_offset[x] |= priority_bit;
                }
                if (pixel) {
                    visibility_offset[x] |= 1 << layer_shift;
                    out[x] = pal | pixel;
                }
                x++;
            }
...
#ifdef USE_BMI2
            uint64_t palette_broadcast = palette * 0x0101010101010101;
#endif
            bool priority = attribute3 & 0x8000;
            uint8_t priority_val = priority ? 1 << LAYER_PRIORITY::SPRITE_HIGH : 1 << LAYER_PRIORITY::SPRITE_LOW;
  ...
  private:
    ...
    static constexpr std::array<std::array<uint32_t, 512>, 3> rgb_entries = make_rgb_entries();
    uint16_t vscroll_a;
    uint16_t vscroll_b;
