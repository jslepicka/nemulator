...
//std::atomic<int> c_ppu<Owner>::lookup_tables_built = 0;
...
//    s_bus<uint16_t> &bus,
//    void * const ctx,
//    clock_callback_t cpu_clock_cb,
//    clock_callback_t ppu_clock_cb,
//    nmi_callback_t nmi_cb,
//    sprite_eval_callback_t sprite_eval_cb,
//    int * const cpu_available_cycles
//    ) :
//    owner(owner),
//    ppu_bus(bus),
//    nes_ctx(ctx),
//    cpu_clock_callback(cpu_clock_cb),
//    ppu_clock_callback(ppu_clock_cb),
//    nmi_callback(nmi_cb),
//    sprite_eval_callback(sprite_eval_cb),
//    cpu_available_cycles(cpu_available_cycles)
//
//{
//    //pSpriteMemory = 0;
//    //mapper = 0;
...
//}
//template<typename Owner>
//uint8_t *c_ppu<Owner>::get_sprite_memory()
//{
//    return sprite_memory;
//}
//template<typename Owner>
//void c_ppu<Owner>::generate_palette()
//{
...
//        static const double levels[8] = {.350, .518, .962, 1.550, 1.094, 1.506, 1.962, 1.962};
//        double lo_and_hi[2] = {levels[level + 4 * (color == 0x0)], levels[level + 4 * (color < 0xd)]};
//        double y = 0.0;
//        double i = 0.0;
//        double q = 0.0;
//
...
//
//        int rgb =
//            0x000001 * (int)(255.0 * srgb_r2) + 0x000100 * (int)(255.0 * srgb_g2) + 0x010000 * (int)(255.0 * srgb_b2);
//
...
//template<typename Owner>
//void c_ppu<Owner>::build_lookup_tables()
//{
//    int expected = 0;
...
//template <typename Owner> c_ppu<Owner>::~c_ppu()
//{
//}

//template<typename Owner>
//INLINE uint64_t c_ppu<Owner>::interleave_bits_64(unsigned char odd, unsigned char even)
//{
//    return morton_odd_64[odd] | morton_even_64[even];
//}

//template <typename Owner> unsigned char c_ppu<Owner>::read_byte(int address)
//{
...
//            if (!rendering)
//                read_value = ppu_bus.read_byte(nes_ctx, vram_address);
//
...
//template <typename Owner> void c_ppu<Owner>::inc_horizontal_address()
//{
...
//template <typename Owner> void c_ppu<Owner>::write_byte(int address, unsigned char value)
//{
//    switch (address) {
//        case 0x2000:    //PPU Control Register 1
//        {
...
//                    }
//                    ppu_bus.read_byte(nes_ctx, vram_address);
//                }
//            }
//            else {
//                if (!rendering) {
...
//template <typename Owner> void c_ppu<Owner>::inc_vertical_address()
//{
...
//template <typename Owner> void c_ppu<Owner>::reset()
//{
//    warmed_up = 0;
//    odd_frame = 0;
//    intensity = 0;
...
//template <typename Owner> INLINE c_ppu<Owner>::s_bg_out c_ppu<Owner>::get_bg()
//{
//    s_bg_out ret = {0, 0};
//
...
//template <typename Owner> INLINE int c_ppu<Owner>::output_pixel_no_sprites()
//{
//    return get_bg().pixel;
//}

//template <typename Owner> INLINE int c_ppu<Owner>::output_pixel()
//{
//    s_bg_out bg_out = get_bg();
//    int pixel = bg_out.pixel;
//    if (sprite_count) {
//        if (sprite_here[current_cycle - screen_offset]) {
//            if (PPUMASK.enable_sprites && ((current_cycle >= 8 + screen_offset) || PPUMASK.left_sprite_enable))
//                [[unlikely]] {
...
//template <typename Owner> INLINE int c_ppu<Owner>::output_blank_pixel()
//{
//    int pixel = 0;
//    if ((vram_address & 0x3F00) == 0x3F00) {
...
//template <typename Owner> void c_ppu<Owner>::begin_vblank()
//{
//    if (warmed_up) {
//        if (!suppress_nmi) {
...
//template <typename Owner> void c_ppu<Owner>::end_vblank()
//{
//    PPUSTATUS.in_vblank = false;
//    nmi_callback(nes_ctx, false);
//}

//template <typename Owner> int c_ppu<Owner>::do_cycle_events()
//{
//    switch (current_cycle) {
//        case 1:
...
//template <typename Owner> INLINE void c_ppu<Owner>::fetch()
//{
...
//            drawing_bg = true;
//            tile = ppu_bus.read_byte(nes_ctx, 0x2000 | (vram_address & 0xFFF));
...
//                __m128i t = _mm_set1_epi8(ppu_bus.read_byte(nes_ctx, attribute_address));
//
...
//            }
//#else
//            attribute = ppu_bus.read_byte(nes_ctx, attribute_address);
...
//            pattern1 = _pdep_u64(ppu_bus.read_byte(nes_ctx, pattern_address),
//                                 0b00000001'00000001'00000001'00000001'00000001'00000001'00000001'00000001);
//#else
//            pattern1 = ppu_bus.read_byte(nes_ctx, pattern_address);
//#endif
//            break;
...
//            drawing_bg = true;
//#ifdef NES_PPU_USE_BMI2
//            pattern2 = _pdep_u64(ppu_bus.read_byte(nes_ctx, pattern_address + 8),
//                                 0b00000010'00000010'00000010'00000010'00000010'00000010'00000010'00000010);
//#else
//            pattern2 = ppu_bus.read_byte(nes_ctx, pattern_address + 8);
//#endif
//
//            {
...
//                ppu_bus.read_byte(nes_ctx, (sprite_tile & 0x1) * 0x1000);
//            }
//            else {
//                ppu_bus.read_byte(nes_ctx, PPUCTRL.sprite_pt_address * 0x1000);
//            }
//            break;
//        case (6 | FETCH_SPRITE):
//            break;
...
//template <typename Owner> void c_ppu<Owner>::run_ppu_line()
//{
//    int pixel_count = 0;
//    int last_cycle = 0;
//    vid_out = 0;
...
//        }
//        pixel_pipeline >>= 8;
//        //Battletoads will enable rendering at cycle 255 and break sprite 0 hit (because vertical update happens on
//        //cycle 256 and misaligns the background). Delaying the update by one cycle seems to fix this.  Unsure if this
//        //is actually how it works, but Mesen source code says that this is what apparently happens.  Need to research
//        //how enabling things mid-screen affects PPU state.
//        //
...
//template <typename Owner> bool c_ppu<Owner>::drawing_enabled()
//{
...
//template <typename Owner> int c_ppu<Owner>::get_sprite_size()
//{
//    return (int)PPUCTRL.sprite_size;
//}

//template <typename Owner> void c_ppu<Owner>::eval_sprites()
//{
//    if (!rendering)
//        return;
//    sprite_here.reset();
//    drawing_bg = false;
//    sprite_eval_callback(nes_ctx, true);
//    int max_sprites = limit_sprites ? 8 : 64;
//    int sprite0_x = -1;
//    sprite0_index = -1;
//    sprite_count = 0;
...
//            if (sprite_height == 16) {
//                sprite_address =
//                    ((sprite_data.tile & 0xFE) * 16) + (sprite_y & 0x07) + ((sprite_data.tile & 0x01) * 0x1000);
//                if (sprite_y > 7)
//                    sprite_address += 16;
//            }
//            else
...
//            uint64_t p1 = _pdep_u64(ppu_bus.read_byte(nes_ctx, sprite_address),
//                                    0b00000001'00000001'00000001'00000001'00000001'00000001'00000001'00000001);
//            uint64_t p2 = _pdep_u64(ppu_bus.read_byte(nes_ctx, sprite_address + 8),
//                                    0b00000010'00000010'00000010'00000010'00000010'00000010'00000010'00000010);
...
//            int *p1 = (int *)morton_odd_64 + (uint64_t)ppu_bus.read_byte(nes_ctx, sprite_address) * 2;
//            int *p2 = (int *)morton_even_64 + (uint64_t)ppu_bus.read_byte(nes_ctx, sprite_address + 8) * 2;
...
//        }
//    }
//    sprite_eval_callback(nes_ctx, false);
//    if (current_scanline == 261)
//        sprite_count = 0;
//    if (sprite_count) {
//        p_output_pixel = &c_ppu::output_pixel;
...
//template <typename Owner> void c_ppu<Owner>::update_vram_address()
//{
//    if (rendering) {
//        inc_horizontal_address();
//        inc_vertical_address();
