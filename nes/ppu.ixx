  ...
  public:
    ...
    void run_ppu_line()
    {
        ...
        while (true) {
            ...
            //cycle 256 and misaligns the background). Delaying the update by one cycle seems to fix this.  Unsure if
            //this is actually how it works, but Mesen source code says that this is what apparently happens.  Need to
            //research how enabling things mid-screen affects PPU state.
            //
    ...
    void generate_palette()
    {
        ...
        for (int pixel = 0; pixel < 512; pixel++) {
            ...
            for (int p = 0; p < 12; ++p) {
                ...
                if (((pixel & 0x40) && wave(p, 12)) || ((pixel & 0x80) && wave(p, 4)) ||
                    ((pixel & 0x100) && wave(p, 8)))
                    spot *= attenuation;
        ...
        //linear RGB -> sRGB
            ...
            int rgb = 0x000001 * (int)(255.0 * srgb_r2) + 0x000100 * (int)(255.0 * srgb_g2) +
                ...
                if (PPUMASK.enable_sprites && ((current_cycle >= 8 + screen_offset) || PPUMASK.left_sprite_enable))
                    ...
                    for (int i = 0; i < num_sprites; i++) {
                        ...
                        if ((unsigned int)(current_cycle - screen_offset - sprite_data.x) < 8) {
                            ...
                            int sprite_color =
                                sprite_index_buffer[i * 8 + (current_cycle - screen_offset - sprite_data.x)];
