  ...
  private:
    ...
    void build_luts()
    {
        ...
        for (int i = 0; i < HalfSineWidth; ++i) {
            ...
            if (!sinx)
                HalfSineLut[i] = MaxAtten;
            else {
                sinx = -20.0 * log10(sinx) * scale;
                if (sinx > MaxAtten)
                    HalfSineLut[i] = MaxAtten;
                else
                    HalfSineLut[i] = static_cast<uint32_t>(sinx);
            }
        }
...
const int c_mapper85::instruments[16][8] = {
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0x21, 0x04, 0x06, 0x8D, 0xF2, 0x42, 0x17, // instrument 1
